...
::
::YAwzoRdxOk+EWAjk
::fBw5plQjdCyDJGyX8VAjFDhbQQGgPWixMLwO/fvHy++UqVkSRN4PVqjn85ehDtwa6UrqOMdj33lV+A==
...
::dhA7pRFwIByZRRnk
::Zh4grVQjdCyDJGyX8VAjFDhbQQGgPWixMLwO/fvHy++UqVkSRN4Xd4LempeBJOUvy1XteZpg0G9JnYY8HhRMexG/Iw4zqGdMpXDIJNeFpA7zTwaM/k5Q
::YB416Ek+ZG8=
::
::
...
:check
REM เช็คว่าไฟล์ "l.exe" ถูกรันหรือไม่
tasklist | find /i "explorer.exe" > nul

REM ตรวจสอบผลลัพธ์ของคำสั่งก่อนหน้า
IF %ERRORLEVEL% EQU 0 (
    REM ถ้าไฟล์ "l.exe" ถูกรันแล้ว
    REM กำหนดคำสั่งที่ต้องการให้ทำต่อไปที่นี่
    echo l.exe ถูกรันแล้ว
    REM ตัวอย่าง: คำสั่งที่ต้องการทำหลังจากเช็คว่า l.exe ถูกรันแล้ว
    goto next
) ELSE (
    REM ถ้าไฟล์ "l.exe" ยังไม่ถูกรัน
    REM กำหนดคำสั่งที่ต้องการให้ทำต่อไปที่นี่
goto check
    REM ตัวอย่าง: คำสั่งที่ต้องการทำหลังจากเช็คว่า l.exe ยังไม่ถูกรัน
)

REM ส่วนนี้จะถูกกระโดดข้ามไปหากไม่มีคำสั่ง goto next ถูกเรียกใช้งาน
:next
echo คำสั่งถัดไป...
REM ต่อๆไปจะเป็นคำสั่งที่ต้องการทำหลังจากการเช็คและดำเนินการเสร็จสิ้น
:getadmin
@echo off
...
IF %ERRORLEVEL% EQU 0 (
cd C:\Report
echo scan with time %date% %time% Have virus >>Scan-By-Apill-fuse-Defender.txt
cd %temp%
) ELSE (
cd C:\Report
echo scan with time %date% %time% not Have virus >>Scan-By-Apill-fuse-Defender.txt
cd %temp%
)
for %%a in (
21
...
set A=%%a
cd C:\Report
echo scan with time %date% %time% kiil port:21, 23, 31, 80, 113, 135, 139, 445, 666, 1001, 1026, 1080, 1170, 1234, 1243, 1363, 1364, 1368, 1373, 1377, 1433, 1434, 1981, 2001, 2023, 2140, 2283, 2535, 2745, 2989, 3024, 3127, 3128, 3131, 3150, 3410, 3700, 4000, 4444, 4950, 5554, 6346, 6400, 6667, 6670, 6969, 7597, 7626, 7777, 8012, 8866, 10000, 10048, 10080, 10100, 11000, 12345, 12346, 12349, 16660, 17300, 18006, 18753, 20034, 20432, 21544, 22222, 23432, 27374, 27665, 30100, 31337, 31338, 31339, 33270, 33567, 33568, 40421, 44444, 60008, 64666, 65000, 65506. >>Protect-by-Apill-fuse-Defender.txt
cd %temp%
timeout 30 /nobreak
goto virus
:error
cd %temp%
echo x=msgbox("your Device is Safety 'start Next scan'", 0, "Protect virus is running") > Realtimecheck.vbs
start Realtimecheck.vbs
timeout 2.1 /nobreak
cd C:\Report
echo your Device is Safety >>scan-data-%date%-%time%.txt
cd %temp%
goto REM
:REM
...
:virus-1
cd %temp%
echo x=msgbox("Not Safety", 0, "Protect virus is running") > Realtimecheck.vbs
start Realtimecheck.vbs
cd C:\Report
goto virus